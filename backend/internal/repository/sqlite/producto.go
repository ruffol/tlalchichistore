...
func NewProductoRepo(db *sql.DB) *ProductoRepo {
	return &ProductoRepo{db: sqlx.NewDb(db, "sqlite")}
}
...
func (r *ProductoRepo) List(ctx context.Context, categoriaSlug string) ([]model.Producto, error) {
	...
	WHERE (? = '' OR c.slug = ?) AND p.activo = 1
	ORDER BY p.nombre`

	var productos []model.Producto
	if err := r.db.SelectContext(ctx, &productos, query, categoriaSlug, categoriaSlug); err != nil {
